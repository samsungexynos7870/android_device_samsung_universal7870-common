...
static int check_ack_val(int fd, unsigned check_val, unsigned size)
{
	...
	/* Get ACK */
	cbd_log("Read ack %dbyte, ack=0x%x\n", size, check_val);
	while (i--) {
...
static int uart_xmit_psi(struct imc_args *args)
{
	...
	err = s_write(fd, "ATAT", 4);
	if (err < 0) {
		cbd_log("send ATAT fail err=%d\n", err);
		goto exit;
	}
	cbd_log("Sent ATAT %d byte\n", err);
	/* Read ack */
	err = s_read(fd, g_rx_buf, 1);
	if (err >= 0 && g_rx_buf[0] == IMC_CHIPID_XMM626X)
...
static int hsic_xmit_main(struct imc_args *args)
{
	...
	if (!tegra) {
		spin = 8;
		while (spin--) {
			cbd_log("-----> Wait Cp wakeup retry count=%d\n", spin);
...
int shutdown_xmm626x_modem(struct boot_args *args)
{
	...
	/* off EHCI ... --- temporary disalbe for Power on/off test */
...
int start_xmm626x_boot(struct boot_args *args)
{
	...
	if (imc_boot.bin_fd < 0) {
		cbd_log("%s open fail err=%d, retry=%d\n", args->cpn->path_bin,
			imc_boot.bin_fd, partition_check--);
		err = imc_boot.bin_fd;
		if (partition_check < 0) {
			cbd_log("CBD stop by partition error\n");
			while (1)
				sleep(0xff);
		}
		goto exit;
...
\return  void
*/
void compute_crc32(unsigned char * ptr, unsigned int length, unsigned char * write_address)
{
	unsigned long crc;
	int c;
...
int check_dump_crc(unsigned long read_crc, char* buf)
{
	unsigned long crc = 0;
...
static char error_info[MAX_ERROR_INFO_BUF_SIZE] = {0,};
