...
static int shannon_boot_xmit_loader(int dev_fd, struct std_boot_args *args, u32 stage)
{
	...
	do {
		...
		ret = s_write(args->dev_fd, img.binary, img.size);
		if (ret < 0) {
			goto exit;
		}
		rest -= len;
	...
	/* wait for cp_status gpio */
	if (wait_gpio_value(PDRIVER_NAME, KEY, GPIO_CP_STATUS, "1", 10) < 0) {
		ret = - ETIMEDOUT;
		goto exit;
	}
exit:
...
static int shannon_normal_boot(struct shannon_boot_args *args)
{
	...
	int fd = std_args->dev_fd;
	enum modem_link link = LINKDEV_LLI;

	/*
	 * set tx_link to LLI. This is required for boot failure cases
	 * Where tx_link starts with SPI instead of LLI.
	 */
	...
	/* set tx_link to SPI before sending boot loader */
	link = LINKDEV_SPI;
	ret = ioctl(fd, IOCTL_MODEM_SET_TX_LINK, &link);
	if (ret < 0)
		goto exit;
	...
	/* set tx_link to lli before sending main binary */
	link = LINKDEV_LLI;
	ret = ioctl(fd, IOCTL_MODEM_SET_TX_LINK, &link);
	if (ret < 0)
		goto exit;
	...
	/*  check for LLI Mount */
	ret = wait_file_value(LLI_STATUS, MOUNT, 10);

	if (ret < 0) {
		cbd_log("LLI STATUS mount failed\n");
		goto exit;
	} else {
		cbd_log("LLI STATUS mount\n");
	}
...
static int shannon_dump_boot(struct shannon_boot_args *args)
{
	...
	/*
	 * set tx_link to LLI. This is required for dump failure cases
	 * Where tx_link starts with LLI instead of SPI.
	 */
	ret = ioctl(fd, IOCTL_MODEM_SET_TX_LINK, &link);
	if (ret < 0)
		goto exit;
...
int start_shannon333_boot(struct boot_args *cbd_args)
{
	int ret = 0;
	int spin = 50;
...
int start_shannon333_dump(struct boot_args *cbd_args)
{
	int ret;
	int log_fd = -1;
	int fd = -1;
...
int shutdown_shannon333_modem(struct boot_args *cbd_args)
{
	int ret = 0;
