...
static int hsic_uart_control(int value);
...
/* C2C boot */
static int c2c_xmit_main(struct imc_args *args)
{
	/*TODO: copy the CP binary to CP MOR*/
	return 0;
}

char cmd_validate_syssw[8] =
	{0x10, 0x00, 0x00, 0x00, 0x08, 0x00, 0x18, 0x00};
char rsp_ok[8] =
	{0x10, 0x00, 0x00, 0x00, 0x00, 0x00, 0x10, 0x00};
char cmd_code_tx_ebl[8] =
	{0x00, 0x00, 0x00, 0x00, 0x02, 0x00, 0x02, 0x00};

static int hsic_usb_host_control(struct imc_args *args, int value);

static int uart_xmit_psi(struct imc_args *args)
{
	...
	/* OPEN LINK PM*/
	link_fd = open(MODEM_LINK, O_RDWR);
	if (link_fd < 0) {
		cbd_log("%s open fail err=%d\n", MODEM_LINK, link_fd);
		err = link_fd;
		goto err_open_linkpm;
	};
	...
	arg = 0;
	err = ioctl(link_fd, IOCTL_LINK_CONTROL_ACTIVE, &arg);
	if (err) {
		cbd_log("host active low  failed: %d\n", err);
		goto exit;
	}

	/* Uart Chennel open */
	fd = open_serial("/dev/ttySAC2", "115200", "N", "8", "1", 0, 0);
	if (fd < 0) {
	...
	err = s_write(fd, cmd_validate_syssw, 8);
	if (err < 0) {
		cbd_log("Send validate_syssw cmd fail err=%d\n", err);
		goto exit;
	}

	/* RSPI_RSP_OK */
	err = s_read(fd, g_rx_buf, 4);
	if (err < 0) {
		cbd_log("Read rpsi_rsp_ok fail err=%d\n", err);
		goto exit;
	}

	err = s_read(fd, &g_rx_buf[4], 4);
	if (err < 0) {
		cbd_log("Read rpsi_rsp_ok fail err=%d\n", err);
		goto exit;
	}

	/* debug dump for cp team */
	for (i = 0; i < 8; i++)
		cbd_log("rpsi_rsp_ok[%d] = 0x%x\n", i, g_rx_buf[i]);

#if 0
	/* check response */
	if (memcmp(g_rx_buf, rsp_ok, 8)) {
		cbd_log("Invalid rpsi_rsp_ok\n");
		goto exit;
	}

	/* RPSI_CMD_CODE_TX_EBL */
	err = s_write(fd, cmd_code_tx_ebl, 8);
	if (err < 0) {
		cbd_log("Send code_tx_ebl fail err=%d\n", err);
		goto exit;
	}

	err = check_ack_val(fd, 0x00aa, 2);
	if (err < 0) {
...
static int hsic_xmit_ebl(struct imc_args *args)
{
	...
	int fd = args->boot_fd;
	char *data;
	int block_size, rest_size, send_size = 0;
	int err;
	...
	while (rest_size) {
		block_size = rest_size > EBL_PAYLOAD_SIZE ? EBL_PAYLOAD_SIZE
			: rest_size;
		err = s_write(fd, data + send_size, block_size);
		if (err < 0) {
			cbd_log("Write failed: %d\n", err);
			goto exit;
		}
...
static int hsic_xmit_xmm7160_modem(struct imc_args *args)
{
#if 0
	 ...
	 * Send SECURE package
	 */
	cbd_log("[xmm7160] Send Main Secure package\n");
	memset(&img, 0x00, sizeof(struct image_buf));
	err = prepare_image(args->bin_fd, &img, &args->img_tab[IMG_SECPACK]);
	if (err < 0) {
			cbd_log("load Main Secure package fail err = %d\n", err);
			goto exit;
	}
	if (img.length != 2048) {
	...
	free(img.buf);
	/* ReqSecStart command */
	cbd_log("Execute ReqSecStart Command [MAIN]\n");
	 ...
	 * Send Main binary
	 */
	cbd_log("[xmm7160] Send Main binary\n");
	memset(&img, 0x00, sizeof(struct image_buf));
	/* check the main binary image map size */
	cbd_log("MAIN imgmap size=0x%x, secpack size=0x%x\n",
		args->img_tab[IMG_MAIN].size, secpack->len);
	if (!args->img_tab[IMG_MAIN].size)
		args->img_tab[IMG_MAIN].size = secpack->len;

	err = prepare_image(args->bin_fd, &img, &args->img_tab[IMG_MAIN]);
	if (err < 0) {
		cbd_log("Load main binary fail\n");
		return -1;
	}
	mem_addr = args->img_tab[IMG_MAIN].mem_offset;
	...
	if (err < 0) {
		cbd_log("cp Main binary send fail err=%d\n", err);
		goto exit;
	}
	free(img.buf);
	...
	/* ReqSecEnd command */
	cbd_log("Execute ReqSecEnd Command [MAIN]\n");
	 ...
	 */
	cbd_log("[xmm7160] Send LTE Secure package\n");
	memset(&img, 0x00, sizeof(struct image_buf));
	 ...
	 * Send LTE binary
	 */
	cbd_log("[xmm7160] Send LTE binary\n");
	memset(&img, 0x00, sizeof(struct image_buf));
	...
	cbd_log("Execute ReqFlashSetAddress cmd addr = 0x%x [LTE]\n", mem_addr);
	err = hsic_execute_cmd(fd, ReqFlashSetAddress, sizeof(unsigned),
		&mem_addr);
	if (err < 0) {
		cbd_log("Excute ReqFlashSetAddr cmd fail err=%d [LTE]\n", err);
		goto exit;
	}
	...
	/* ReqSecEnd command */
	cbd_log("Execute ReqSecEnd Command [LTE]\n");
	err = hsic_execute_cmd(fd, ReqSecEnd, sizeof(unsigned short),
			   &sec_end);
	if (err < 0)
		goto exit;
...
/* Uart boot */
static int uart_xmit_boot(struct imc_args *args)
{
	int err;
	int fd = args->boot_fd;
	...
	/*
	 * Send AT and PSI binary to xmm626x through UART
	 */
	cbd_log("PSI send ----\n");
	err = uart_xmit_psi(args);
	if (err < 0) {
...
static int hsic_uart_control(int value)
{
	...
	/* OPEN EHCI */
	uart_fd = open("/sys/class/sec/uart_cp/control", O_RDWR);
	if (uart_fd < 0) {
		cbd_log("uart_cp open fail err=%d\n", uart_fd);
		err = uart_fd;
		goto exit;
	}

	if (value) {
		err = write(uart_fd, "1", strlen("1"));
		if (err < 0) {
			cbd_log("uart write value=%s failed: %d\n", "1", err);
			goto exit;
		}
	} else {
		err = write(uart_fd, "0", strlen("0"));
		if (err < 0) {
			cbd_log("uart write value=%s failed: %d\n", "0", err);
			goto exit;
		}
...
/* HSIC boot */
static int hsic_xmit_boot(struct imc_args *args)
{
	int err, connected = 0, spin;
	int fd = args->boot_fd, link_fd;
	int tegra = !!(args->cbd_args->options & BOPT_EHCI_TEGRA);
	...
	spin = 10;
	while (spin--) {
		connected = ioctl(link_fd, IOCTL_LINK_CONNECTED);
		cbd_log("check [BOOTROM] link connected = [%d]\n",
			connected);
		if (connected == 1)
			break;
		usleep(500000);
...
static int hsic_xmit_main(struct imc_args *args)
{
	int err, arg, connected = 0, spin;
	int link_fd;
	int tegra = !!(args->cbd_args->options & BOPT_EHCI_TEGRA);
	struct boot_args *cbd_arg = args->cbd_args;
	int fd = args->boot_fd;
	...
	while (spin--) {
		connected = ioctl(link_fd, IOCTL_LINK_CONNECTED);
		cbd_log("check [PSI] link connected = [%d]\n", connected);
		if (connected == 1)
			break;
		usleep(500000);
	}
	...
	if (cbd_arg->type == IMC_XMM7160)
		err = hsic_xmit_xmm7160_modem(args);
	else
	...
	if (!tegra) {
		spin = 16;
		while (spin--) {
			cbd_log("-----> Wait Cp wakeup high retry count=%d\n", spin);
			err = ioctl(link_fd, IOCTL_LINK_GET_HOSTWAKE);
			if (err == 0) {
				cbd_log("-----> CP HOST wakeup\n");
				break;
			}
			usleep(500000);
		}
		...
		if (err < 0)
			goto exit;
		cbd_log("boot: EHCI off-on for re-enumeration\n");
	}

	arg = 0;
	err = ioctl(link_fd, IOCTL_LINK_CONTROL_ENABLE, &arg);
	if (err) {
		cbd_log("ehci_en off  failed: %d\n", err);
		goto exit;
	}
	...
	spin = 16;
	while (spin--) {
		cbd_log("-----> Wait Cp wakeup low retry count=%d\n", spin);
		err = ioctl(link_fd, IOCTL_LINK_GET_HOSTWAKE);
		if (err == 1) {
			cbd_log("-----> CP HOST wakeup\n");
			break;
	...
	err = ioctl(link_fd, IOCTL_LINK_CONTROL_ACTIVE, &arg);
	if (err) {
		cbd_log("host active high  failed: %d\n", err);
		goto exit;
	}
	...
	while (spin--) {
		connected = ioctl(link_fd, IOCTL_LINK_CONNECTED);
		cbd_log("check [MAIN] link connected = [%d] cnt=%d\n",
							connected, spin);
		if (connected == 1)
			break;
		usleep(500000);
...
int start_xmm72xx_boot(struct boot_args *args)
{
	...
	case LINKDEV_C2C:
		err = c2c_xmit_main(&imc_boot);
		if (err < 0) {
			cbd_log("hsic_xmit_main fail (err=%d)\n", err);
			goto exit;
		}
		break;
	case LINKDEV_LLI:
...
static int check_dump_crc(unsigned long read_crc, char* buf)
{
	...
	if (crc != read_crc) {
		cbd_log("read_crc : 0x%08lx, dump_crc : 0x%08lx\n", crc, read_crc);
		return -1;
	}
	else
		return 1;
...
int start_xmm72xx_dump(struct boot_args *args)
{
	...
	err = s_write(fd, "PASS", 4 * sizeof(unsigned char));
	if (err < 0) {
		cbd_log("Write failed: %d\n", err);
		goto exit;
	}
