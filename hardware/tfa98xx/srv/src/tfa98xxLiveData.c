...
Tfa98xx_Error_t tfa98xx_get_live_data_raw(Tfa98xx_handle_t handle, unsigned char *bytes, int length)
{
        ...
        /* before getting memtrack items, check if we can access the DSP */
...
Tfa98xx_Error_t tfa98xx_set_live_data(Tfa98xx_handle_t *handlesIn)
{
	Tfa98xx_Error_t err = Tfa98xx_Error_Ok;
        nxpTfaContainer_t *cont;
        nxpTfaLiveDataList_t *lData;
	...
	if ( tfa98xx_dev_family(handlesIn[0]) == 2) {
		...
		if(cont->nliveData > 0) {
			...
			/* Check if CF is on! (Does not mean we are in operating mode!) */
			if (tfa98xx_cf_enabled(handlesIn[0])) {
				create_memtrack_buffer_msg(&mRecord, buffer, &size);
				/* We dont need to know the error!
				 * Because if the CF is off should/can still print DATASHEET names
				 */
				tfa_dsp_msg(handlesIn[0], size, buffer);
			} else {
				PRINT("\nWarning: CoolFlux_DSP is off. All items requiring CF interaction are printed with 0 values! \n\n");
			}
		} else {
			/* There is no memtrack section in the ini file.
			 * Information is already provided in the function tfa98xx_get_live_data_items
			 */
...
Tfa98xx_Error_t tfa98xx_get_live_data(Tfa98xx_handle_t *handlesIn, int idx, float *live_data, int *nr_of_items)
{
	...
	char *order[MEMTRACK_MAX_WORDS];
	unsigned char inforegs[6];
	int CFstatus=0, supportDrc, listNumber=0;
        unsigned int i, j, stateSize;
	...
	if ( tfa98xx_dev_family(handlesIn[idx]) == 2) {
		 ...
		 */
		for(i=0;i<lData->length-1u;i++) {
			if(strcmp(order[i], "M") == 0) {
				/* CF is on */
				if (CFstatus) {
				...
				bf.field = tfaContBfEnum(order[i], tfa98xx_dev_revision(handlesIn[idx])); //TODO solve for max1
				err = tfaRunReadBitfield(handlesIn[idx], &bf);
				live_data[i] = (float)bf.value / mRecord.scalingFactor[i];
			}
		}

		*nr_of_items = lData->length-1;
	} else { /* Max1 devices */
		...
		if (CFstatus) {
			...
			if(err == Tfa98xx_Error_Ok) {
				...
				* some features aren't supported */
				...
				if (tfa98xx_dev_revision(handlesIn[idx]) != 0x97) {
			...
			if(lData != NULL) {
				...
				for(j=0; j<lData->length-1u; j++) {
					...
					if(state_info_memtrack_compare(liveD->name, &listNumber)) {
						...
						/* Get the Bitfield value */
						bf.field = tfaContBfEnum(order[j], tfa98xx_dev_revision(handlesIn[idx]));
						err = tfaRunReadBitfield(handlesIn[idx], &bf);
...
int tfa98xxGetWaveformValues(int handle, int *data, double *waveform, int xmodel)
{
	...
	int i, max=0, max_index=0, lagW = 12, stateSize = LSMODEL_MAX_WORDS;
	...
	s_subBandDynamics *sbDynamics, float *SbGains_buf, float *SbFilt_buf, float *CombGain_buf, int maxlength)
{
	...
	double subBandFiltFreqTrFunc[MAX_NR_SUBBANDS][FFT_SIZE_DRC/2] = {0};
	double subBandGainFreqTrFunc[MAX_NR_SUBBANDS][FFT_SIZE_DRC/2] = {0};
