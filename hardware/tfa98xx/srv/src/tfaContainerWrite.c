...
static void convert_step(nxpTfaOldVolumeStep2_t *old, nxpTfaVolumeStep2_t *newVS) {
	...
	for(i=0;i<TFA98XX_MAX_EQ;i++) {
		...
		newVS->filter[i].type = (uint8_t)old->eq[i].type;
...
int tfaContBin2Hdr(char *inputname, int argc, char *argv[]) {
	...
	i=0;
	if(cntFile) {
		if (argc > 0) strncpy(thisCont->customer, argv[i++], sizeof(thisCont->customer)-1);
		if (argc > 1) strncpy(thisCont->application, argv[i++], sizeof(thisCont->application)-1);
		if (argc > 2) strncpy(thisCont->type, argv[i++], sizeof(thisCont->type)-1);
	} else {
		if (argc > 0) strncpy(hdr->customer, argv[i++], sizeof(hdr->customer)-1);
		if (argc > 1) strncpy(hdr->application, argv[i++], sizeof(hdr->application)-1);
		if (argc > 2) strncpy(hdr->type, argv[i++], sizeof(hdr->type)-1);
	}
...
static int tfaContAddItem(nxpTfaContainer_t *cont, nxpTfaDescPtr_t *dsc, nxpTfaLocationInifile_t *loc, int devidx)
{
	...
	case dscLiveDataString:
		...
		for (i = 2; i < n_spaces; ++i) {
			if(res[i] != NULL)
				sscanf(res[i], "0x%x", &(liveD.scalefactor[i-2]));

			if(i-2 > 9) {
				PRINT("\nError: The payload is to big. \n");
				free (res);
				return 1;
			}
		}
		...
		liveD.liveData_size = (uint8_t)n_spaces - 2;
        size = sizeof(liveD.liveData_size) + sizeof(liveD.name) + sizeof(liveD.addrs) +
                       sizeof(liveD.tracker) + (sizeof(liveD.scalefactor[0]) * (n_spaces-2));
	...
	case dscCfMem: /* @x[add]=val0[,val1,..valn] */
		...
		if ( (sscanf(&str[2], "[0x%x]=", (unsigned int *)&address)==1) ||
				(sscanf(&str[2], "[%d]=", (int *)&address)==1) )
		{
...
/* find device type from patch file(s) first */

static int tfaContFindDevtypes(const char *inifile)
{
	FILE *in;
	char buf[MAXLINELEN], *ptr;
	int cnt = 0,ret=-1;
	char p[MAXLINELEN];
	char pathname[FILENAME_MAX],fname[FILENAME_MAX];
	...
	while (!feof(in)) {
		...
		if (strncmp(ptr, "patch", 5 ) == 0) {
			if (sscanf(ptr,"patch=%255s\n", p)==1) {
				tfaGetAbsolutePath((char*)inifile, (nxpTfaLocationInifile_t*)fname);
				strcat(fname, p);

				tfaGetAbsolutePath(fname, (nxpTfaLocationInifile_t*)pathname);
				//PRINT("dev:%d %s \n",cnt,fname);
				if (tfa_cont_set_patch_dev(pathname, basename(fname), cnt) <0 ) {
					break;
				}
				cnt++;
			} else {
				PRINT("unexpected patchfile syntax %s", ptr);
			}
			ret = 0;
		}
	}
...
int tfaContParseIni( char *iniFile, char *outFileName, nxpTfaLocationInifile_t *loc)
{
	...
	/* find device type from patch file(s) first */
	if (tfaContFindDevtypes(iniFile))
		return 0;
	...
	for (i = 0; i < keys.n; i++) {
		...
		for (k = 0; ini_getkey(keys.names[i], k, key, sizearray(key), preformatIni) > 0; k++) {
			...
			case dscBitfield:
				/* if not in any list */
				if ( tfaContBfEnumAny(&key[4])==0xffff ) { //TODO get device idx for type
					PRINT("unknown bitfield: %s \n", key);
					error = 1;
				}
#if 1 // TODO: remove me
	...
	for (i = 0; i < keys.n - 1; i++) {	// loop one less from total, 1st is done
		...
		tfaContSetOffset(dsc, idx + keys.n + sizeof(nxpTfaDeviceList_t) / 4); // the device list offset is updated
	}
	...
	*/
	...
	for (i = 0; i < profiles.n; i++) {
		...
		for (k = 0; ini_getkey(profiles.names[i], k, key, sizearray(key), preformatIni) > 0; k++) {
			...
			case dscBitfield:
				/* if not in any list */
				if ( (tfaContBfEnum(&key[4], (unsigned short)tfa_dev_type[0]) == 0xffff)  ) {
					PRINT("unknown bitfield: %s \n", key);
					error = 1;
				}
			case dscSetProgramConfig:
	...
	headAndDescs->nliveData = (uint16_t)livedata.n; // record nr of livedata
        if(livedata.n > 0) {
			...
			if(k > MEMTRACK_MAX_WORDS) {
                ...
                dsc->type = dscMarker;	// mark end of proflists
	        dsc->offset = 0x777777;	// easy to read
	        dsc++;
        }
...
int tfa98xx_cnt_split(char *fileName, char *directoryName) {
	...
	unsigned int m=0;
	char *filename;
	char buffer[20*1024];	//,byte TODO check size or use malloc
...
#if defined (WIN32) || defined(_X64)
		mkdir(loc->locationIni);
#else
		mkdir(loc->locationIni, 0777);
#endif
	}
			...
			if ( dev->list[j].type == dscFile || dev->list[j].type == dscPatch) {
				...
				if(strcmp(tfaContFileTypeName(file), "info") == 0) {
					error = tfaosal_filewrite(completeName,
						(unsigned char *)buf+sizeof(nxpTfaHeader_t), file->size-sizeof(nxpTfaHeader_t));
				} else {
					error = tfaosal_filewrite(completeName, (unsigned char *)buf, file->size );
				}
...
enum Tfa98xx_Error tfaContShowItem(nxpTfaDescPtr_t *dsc, char *strings, int maxlength, int devidx)
{
	...
	case dscCfMem:
		...
		str2 = str;
		/* @y[10]=0x1234,0xaaaaaa,1234,5678 prints as:
			* @y[0x0a]=0x1234,0xaaaaaa,0x4d2,0x162e
			* */
		switch (mem->type) {
		...
		default:
			...
			goto break_goto; /* no further processing */
			break;
		}
	...
	case dscLiveDataString:
		...
		sprintf(str, "%s=%s, 0x%02x", liveD->name, liveD->addrs, liveD->tracker);
		err = tfa_append_substring(str, strings, maxlength);

		for(i=0; i<liveD->liveData_size; i++) {
			sprintf(str, ", 0x%x", liveD->scalefactor[i]);
			err = tfa_append_substring(str, strings, maxlength);
		}
		sprintf(str, "\n");
		break;
 ...
 */
TFA2_NAMETABLE
static int tfa2RunBitfieldDump(unsigned char reg, unsigned short regval ) {
	union {
		uint16_t field;
		nxpTfaBfEnum_t Enum;
	} bfUni;
	uint16_t 	mask;
	int n=sizeof(Tfa2DatasheetNames)/sizeof(tfaBfName_t)-1; // end of list
	int havename=0;
	do {
		bfUni.field = Tfa2DatasheetNames[n].bfEnum;
		if (reg==0x73){
			mask=1;
		}
		if ( bfUni.Enum.address == reg ){
			mask = (1<<(bfUni.Enum.len+1))-1;
			PRINT("%s:%d ", Tfa2DatasheetNames[n].bfName, (regval>>bfUni.Enum.pos) & mask);
			havename=1;
		}
	}
	while( n--);
...
TFA1_NAMETABLE //TODO move all bitfield code to a single source file
TFA9891_NAMETABLE
TFA9890_NAMETABLE
TFA9887_NAMETABLE
void fillBitFieldNameTable(unsigned short bitValues[16], char bitFieldNames[16][16], tfaBfName_t *datasheetNameTable,
                            int tableLength, unsigned char reg, unsigned short regval)
{
	union {
		uint16_t field;
		nxpTfaBfEnum_t Enum;
	...
	uint16_t mask;
	int tableIndex;
	unsigned int regIndex, offset;

	for(regIndex=0; regIndex<=15; regIndex++){
		for(tableIndex=0; tableIndex<tableLength; tableIndex++){
			bfUni.field = datasheetNameTable[tableIndex].bfEnum;
			if (bfUni.Enum.address == reg && bfUni.Enum.pos == regIndex) {

                                /* clear all previous (default TFA1) entries with the current datasheetName: */
                                for(offset=0; offset<=15; offset++) {
                                        if( strcmp(datasheetNameTable[tableIndex].bfName, bitFieldNames[offset]) == 0 ) {
                                                bitFieldNames[offset][0] = '\0';
                                                bitValues[offset] = 0;
                                        }
                                }

                                /* save bitfield names and values: */
				mask = (1<<(bfUni.Enum.len+1))-1;
				for(offset=0; offset<=bfUni.Enum.len; offset++){
					strcpy(bitFieldNames[regIndex+offset], datasheetNameTable[tableIndex].bfName);
					bitValues[regIndex+offset] = (regval>>bfUni.Enum.pos) & mask;
				}
			}
		}
	}
...
static int tfa1RunBitfieldDump(unsigned char reg, unsigned short regval) {
	int regIndex;
	int tableLength=0;
	int rev = tfa98xx_dev_revision(0); /* In order to diff between 87, 90 and other max1 */
	char bitFieldNames[16][16] = {{'\0', '\0'}};
	char lastFieldName[16] = {'\0'};
	unsigned short bitValues[16] = {0};

	if  ( rev != 0x92 ) {
	/* write the general TFA1 registers: */
	tableLength = sizeof(Tfa1DatasheetNames)/sizeof(tfaBfName_t); /* end of list */
	fillBitFieldNameTable( bitValues, bitFieldNames, Tfa1DatasheetNames, tableLength, reg, regval);
	} else {
		/* the tfa9891 has the full table, this may be optimized */
		tableLength = sizeof(Tfa9891DatasheetNames)/sizeof(tfaBfName_t); /* end of list */
		fillBitFieldNameTable( bitValues, bitFieldNames, Tfa9891DatasheetNames, tableLength, reg, regval);
	}
	/* overwrite the device specific registers for tfa9887(B) and tfa9890(B): */
	if(rev == 0x12){
		tableLength = sizeof(Tfa9887DatasheetNames)/sizeof(tfaBfName_t); /* end of list */
		fillBitFieldNameTable( bitValues, bitFieldNames, Tfa9887DatasheetNames, tableLength, reg, regval );
	}else if((rev == 0x80) || (rev == 0x91)){
		tableLength = sizeof(Tfa9890DatasheetNames)/sizeof(tfaBfName_t); /* end of list */
		fillBitFieldNameTable( bitValues, bitFieldNames, Tfa9890DatasheetNames, tableLength, reg, regval );
	}

	/* print the registers: */
	for(regIndex=15; regIndex>=0; regIndex--){
		if(strlen(bitFieldNames[regIndex]) != 0){
			if( strcmp(bitFieldNames[regIndex], lastFieldName) != 0 ){
				PRINT("%s:%d ", bitFieldNames[regIndex], bitValues[regIndex]);
			}
		}
		strcpy(lastFieldName, bitFieldNames[regIndex]);
	}
	PRINT("\n");
...
int tfaRunBitfieldDump(unsigned char reg, unsigned short regval, int  family) {

	if (family==2)
		return tfa2RunBitfieldDump(reg, regval);
	else if (family==1)
		return tfa1RunBitfieldDump(reg, regval);
	else {
		PRINT_ERROR("Device family type not known!\n");
	}
