...
/**
 * Calculate the tCoef value
 */
float tfa1CaltCoefFromSpeaker(unsigned char *speakerBytes)
{
	int iCoef;

	/* tCoef(A) is the last parameter of the speaker */
	iCoef = (speakerBytes[TFA1_SPEAKERPARAMETER_LENGTH-3]<<16) + (speakerBytes[TFA1_SPEAKERPARAMETER_LENGTH-2]<<8) + speakerBytes[TFA1_SPEAKERPARAMETER_LENGTH-1];

	return (float)iCoef/(1<<23);
}
void calculate_tCoef_calibration(unsigned char *buffer, float *tCoef)
{
        //int data[150];

        *tCoef = tfa1CaltCoefFromSpeaker(buffer);
        return ; //TODO FIX THIS! MAX1

	//tfa98xx_convert_bytes2data(3 * LSMODEL_MAX_WORDS, buffer, data);

        //*tCoef = (float)data[tCoef_IDX] / (float)tCoef_SCALE;
}
